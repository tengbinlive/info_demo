...
public class ActMyPartakeDetailActivity extends AbsDetailActivity {
    ...
    private MyPartakeInvActBean invActBean;
    ...
    @Override
    public void EInit() {
        invActBean = (MyPartakeInvActBean) getIntent().getSerializableExtra(KEY_DETAIL);
        super.EInit();
        findView();
        initmScrollView();
        getDetail();
    ...
    private void getSign() {
        manager.actSign(this, invActBean.getActvid(), "" + invActBean.getActivity().getCharge(), activityHandler, LOAD_SIGN);
    }
    ...
    private void findView() {
        ...
        initWebView(invActBean.getActivity().getH5url());
        ...
        String joinStatus = invActBean.getActivity().getIsJoin();
        String pubsid = invActBean.getActivity().getPubsid();
    ...
    public ArrayList<BottomMenu> getMenuEnums() {
        ...
        if (invActBean.getActivity().getPubsid().equals(App.getInstance().getgUserInfo().getServno())) {
    ...
    private void toReview(MyPartakeInvActBean bean) {
    ...
    private void selectPict(MyPartakeInvActBean bean) {
        if (mBottomView != null) {
