...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private boolean mMaxPullHeightEnable = false;
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		if (!isPullToRefreshEnabled() || !isLoadingData) {
			return false;
		}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullToRefresh_ptrMode, 2));
		}
...
//		}
		mOverScrollEnabled = false;//禁止所有的回弹效果
		...
		if(a.hasValue(R.styleable.PullToRefresh_ptrHasMaxPullHeight)) {
			mMaxPullHeightEnable = a.getBoolean(
					R.styleable.PullToRefresh_ptrHasMaxPullHeight, false);
		}
	...
	private void pullEvent() {
		...
		if(mMaxPullHeightEnable) {
			setHeaderScroll(newScrollValue >= itemDimension? itemDimension : newScrollValue);   //设定上拉的最大高度
		} else {
